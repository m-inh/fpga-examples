...
) 
{
    ...
    for (i=0; i<32; i++) {
        ...
        plain_fpid[plain_fpid_offset + i] = dwt_eco1;
    }
    ...
    for (i=0; i<32; i++) {
        wave_index = plain_fpid_offset + i;
        
        if (wave_index + 1 < NUMDWTECO) {
            if (plain_fpid[wave_index] < plain_fpid[wave_index + 1]) {
    ...
    for (i=0; i<32; i++) {
        ...
        plain_fpid[wave_index] = local_orientations[i];
    }
    ...
    {   
        temp_fpid <<= 1;
        // fpid[g_id] <<= 1;
        plain_fpid_index = plain_fpid_offset + i;
